...
int socket_create(protocol_t protocol_type, int* socket_fd_ptr) {
  ...
  if (!ret && ((*socket_fd_ptr) < 0)) {
    ret = -1;
    log_msg(ERROR, STANDARD, "Setup Socket (socket)", strerror(errno));
  }
...
int socket_bind(const socket_address_t* local_address, int socket_fd) {
  ...
  if (bind(socket_fd, (struct sockaddr*) local_address,
    ...
    ret = -1;
    log_msg(ERROR, STANDARD, "Setup Socket (bind)", strerror(errno));
  }
...
int socket_listen(int socket_fd) {
  ...
  if (listen(socket_fd, 5) < 0) {
    log_msg(ERROR, STANDARD, "Setup Socket (listen)", strerror(errno));
    ret = -1;
  }
  return ret;
}
...
int server_handler(int master_socket_fd, policy_t accept_error_policy,
  ...
  if (getsockopt(master_socket_fd, SOL_SOCKET, SO_ACCEPTCONN, &accept_conn,
    ...
    ret = -1;
    log_msg(ERROR, STANDARD, "Server Handler",
        "Cannot check if the socket is listening");
  } else if (accept_conn) {
    log_msg(DEBUG, STANDARD, "Server Handler", "The socket is listening");
  } else {
    ret = -2;
    log_msg(ERROR, STANDARD, "Server Handler", "The socket isn't listening");
  }
  // }
  ...
  while(TRUE) {
    ...
    if (slave_socket_fd < 0) {
      if (errno == EINTR) { // System call interrupted
        log_msg(WARNING, STANDARD, "Server Handler",
        ...
        if (accept_error_policy == STOP) { // STOP policy
          ...
          log_msg(WARNING, STANDARD, "Server Handler",
              "Skipped the current client connection because of a bad accept");
          continue;
        }
      }
    }
    ...
    if (!ret) {
      ...
      if (slave_pid < 0) {
        if ((*fork_error_handler)(slave_socket_fd, &client_address) < 0) {
          ...
          log_msg(ERROR, STANDARD, "Server Handler", "Handled the fork error");
        }
      } else if (slave_pid == 0) {
        if (close(master_socket_fd) < 0) {
          log_msg(WARNING, STANDARD, "Slave", "Failed to close"
              " the master socket: fd=%d", master_socket_fd);
        }
        ret = (*slave_logic)(slave_socket_fd, &client_address);
        if (ret) {
          log_msg(ERROR, STANDARD, "Slave", "Failed to handle the client");
        }
        if (close(slave_socket_fd) < 0) {
          log_msg(WARNING, STANDARD, "Slave", "Failed to close"
              " the slave socket: fd=%d", slave_socket_fd);
        }
        exit((ret == 0) ? EXIT_SUCCESS : EXIT_FAILURE);
      } else {
        errno = 0;
        if (close(slave_socket_fd) < 0) {
          log_msg(WARNING, STANDARD, "Server Handler", "Failed to close"
              " the slave socket: fd=%d", slave_socket_fd);
        }
      }
    }
...
int client_handler(int client_socket_fd, socket_address_t master_address, unsigned int retry_count) {
  int ret = 0;
  int accept_conn;
  socklen_t accept_conn_length = sizeof(accept_conn);
  ...
  if (getsockopt(client_socket_fd, SOL_SOCKET, SO_DEBUG, &accept_conn,
        &accept_conn_length) < 0) {
    ret = -1;
    log_msg(ERROR, STANDARD, "Client Handler", "The socket is invalid");
  } else {
    log_msg(DEBUG, STANDARD, "Client Handler", "The socket is valid");
  }
  // }
  ...
  if (!ret) {
    errno = 0;
    if (connect(client_socket_fd, (struct sockaddr*) &master_address, sizeof(master_address)) < 0) {
      ...
      log_msg(DEBUG, STANDARD, "Client Handler", "Connected to the socket: %d:%d", master_address.sin_addr, master_address.sin_port);
    }
  }
  // }
  ...
  if (close(client_socket_fd) < 0) {
    log_msg(WARNING, STANDARD, "Client Handler", "Failed to close"
        " the client socket: fd=%d", client_socket_fd);
  }
